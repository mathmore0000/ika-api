...
COMMENT ON COLUMN audit.logged_actions.session_user_id IS 'User id whose statement caused the audited event';
...
BEGIN
    ...
    audit_row = ROW(
        ...
        current_setting('myapp.user_id', true),                   -- session_user_id
...
select audit.audit_table('public.active_ingredient');
select audit.audit_table('public.category');
select audit.audit_table('public.labels');
select audit.audit_table('public.medication');
select audit.audit_table('public.usage');
select audit.audit_table('public.usage_labels');
select audit.audit_table('public.user_medication');
select audit.audit_table('public.user_medication_stock');
select audit.audit_table('public.user_medication_stock_usage');
select audit.audit_table('public.user_responsibles');

select audit.audit_table('storage.buckets');
select audit.audit_table('storage.files');